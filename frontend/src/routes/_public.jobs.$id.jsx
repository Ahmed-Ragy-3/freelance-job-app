...
import { bookmarkService, staticData, profileService, applicationService } from "@/services";
...
import { Bookmark, Share2, Clock, Users, MapPin, Calendar, Paperclip, Zap, TrendingUp, Undo2 } from "lucide-react";
...
const canWithdrawStatus = (status) =>
  ["In_Progress", "Draft", "Pending", "Submitted"].includes(status);
...
function JobDetail() {
  ...
  const { data: client } = useAsync(
    () => (job?.clientId ? profileService.getClient(job.clientId) : Promise.resolve(null)),
    [job?.clientId]
  );
  ...
  const refreshApplication = useCallback(() => {
    ...
    return applicationService
      .getByFreelancerAndJob(user.id, id)
      .then(setExistingApp)
      .catch(() => setExistingApp(null))
      .finally(() => setAppLoading(false));
  }, [user, isFreelancer, id]);

  useEffect(() => {
    if (user && id) bookmarkService.isBookmarked(user.id, id).then(setBookmarked);
  }, [user, id]);
  ...
  const onBookmark = async () => {
    ...
    const { bookmarked: next } = await bookmarkService.toggle(user.id, id);
    setBookmarked(next);
  ...
  const jobOpen = job.status === "Open" || job.status === "Approved";
  const tagItems = (job.tags || []).map((t) =>
    typeof t === "string" ? { key: t, label: t } : { key: t.id ?? t.name, label: t.name || "" }
  ).filter((t) => t.label);
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
            ...
            <div className="p-6">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {hasApplied && <Badge variant="primary">Applied</Badge>}
                {cat && <Badge variant="outline">{cat.name}</Badge>}
                {tagItems.map((t) => (
                  <Badge key={t.key} variant="primary">{t.label}</Badge>
                ))}
              </div>
              ...
              <div className="mt-6">
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {job.requiredSkills?.map((s) => (
                    <Badge key={typeof s === "string" ? s : s.id || s.name} variant="primary">
                ...
                <div className="mt-6">
                  ...
                  <ul className="mt-2 space-y-1">
                    {job.attachments.map((a, i) => (
                      <li key={i} className="inline-flex items-center gap-2 text-sm text-primary hover:underline">
                        <Paperclip size={14} />{typeof a === "string" ? a : a.fileName || a.url}
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="mt-6 rounded-2xl border border-border bg-card p-6 shadow-soft">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              {!similar || similar.length === 0
                ? <p className="text-sm text-muted-foreground">No similar jobs yet.</p>
        ...
        <aside className="space-y-4">
          <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
            <div className="text-sm text-muted-foreground">
              {job.projectType === "Hourly" ? "Hourly rate" : "Fixed budget"}
            </div>
            ...
            <div className="mt-5 space-y-2">
              {!isAuthenticated ? (
                ...
                <>
                  {hasApplied ? (
                    <Button className="w-full" disabled title="You already applied for this job">
                      {appLoading
                        ? "Checking…"
                        : `Applied${existingApp.status ? ` · ${String(existingApp.status).replace("_", " ")}` : ""}`}
                    </Button>
                  ) : (
                    <Button
                      className="w-full"
                      disabled={!jobOpen || appLoading}
                      onClick={() => navigate({ to: "/jobs/$id/apply", params: { id } })}
                    >
                      {appLoading ? "Checking…" : "Apply now"}
                    </Button>
                  )}
                  {hasApplied && canWithdrawStatus(existingApp.status) && (
                    <Button
                      className="w-full"
                      variant="danger"
                ...
                <Button className="w-full" disabled title="Only freelancers can apply">
                  Only freelancers can apply
                </Button>
              )}
              <div className="grid grid-cols-2 gap-2">
                <Button variant="outline" onClick={onBookmark}>
                  <Bookmark size={14} className={bookmarked ? "fill-primary text-primary" : ""} />
          ...
          <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
            ...
            <div className="mt-4 flex items-center gap-3">
              <Avatar
                src={client?.logo || `https://i.pravatar.cc/200?u=${job.clientId}`}
            ...
            <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
              <li className="flex justify-between">
                <span className="inline-flex items-center gap-1"><TrendingUp size={12} />Jobs posted</span>
                <span className="font-medium text-foreground">{client?.jobsPosted ?? "—"}</span>
              </li>
              <li className="flex justify-between">
                <span>Hire rate</span>
                <span className="font-medium text-foreground">{client?.hireRate ?? "—"}%</span>
              </li>
              <li className="flex justify-between">
                <span className="inline-flex items-center gap-1"><Zap size={12} />Response time</span>
                <span className="font-medium text-foreground">{client?.responseTime ?? "—"}</span>
              </li>
              <li className="flex justify-between">
                <span>Member since</span>
                <span className="font-medium text-foreground">{client?.memberSince ?? "—"}</span>
              </li>
              <li className="flex justify-between">
                <span>Location</span>
