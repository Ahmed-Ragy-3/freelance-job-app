...
export const jobs = Array.from({ length: 18 }).map((_, i) => {
  ...
  return {
    ...
    clientId,
    categoryId: cat.id,
    categoryIds: [cat.id, categories[(i + 2) % categories.length].id],
    categoryNames: [cat.name, categories[(i + 2) % categories.length].name],
...
export const applications = [
  { id: "a1", jobId: "j1", freelancerId: "u1", coverLetter: "I've built 3 dashboards like this at scale and can start immediately.", bid: 4200, timeline: "3 weeks", status: "Accepted", createdAt: daysFromNow(-2) },
  { id: "a2", jobId: "j2", freelancerId: "u6", coverLetter: "Redesigned similar onboarding for two SaaS companies — happy to share case studies.", bid: 2800, timeline: "2 weeks", status: "Accepted", createdAt: daysFromNow(-4) },
  ...
  { id: "a4", jobId: "j5", freelancerId: "u6", coverLetter: "Design systems for fintech is my niche.", bid: 5400, timeline: "5 weeks", status: "Accepted", createdAt: daysFromNow(-8) },
  ...
  { id: "a6", jobId: "j4", freelancerId: "u2", coverLetter: "Kubernetes migrations are my day job — happy to share a rollout plan.", bid: 7400, timeline: "8 weeks", status: "Accepted", createdAt: daysFromNow(-12) },
];

// Job lifecycle overrides so the review flow has realistic data:
// completed jobs (reviewable) and one in-progress job the client can complete.
const setJobStatus = (id, status) => { const j = jobs.find((x) => x.id === id); if (j) j.status = status; };
setJobStatus("j1", "In Progress");
setJobStatus("j2", "Completed");
setJobStatus("j3", "Completed");
setJobStatus("j4", "Completed");
setJobStatus("j5", "Completed");
...
export const reviews = [
  ...
  { id: "r2", jobId: "j2", from: "u4", to: "u6", rating: 5, text: "Maya's designs elevated our product. Best hire this year.", createdAt: daysFromNow(-14) },
  { id: "r3", jobId: "j4", from: "u4", to: "u2", rating: 4, text: "Great technical work, communication could be slightly quicker.", createdAt: daysFromNow(-30) },
];
